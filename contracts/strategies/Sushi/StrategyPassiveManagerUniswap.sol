...
import {V3StratFeeManagerInitializable, IFeeConfig} from "../Common/V3StratFeeManagerInitializable.sol";
...
import {IPlexusVaultConcLiq} from "../../interfaces/plexus/IPlexusVaultConcLiq.sol";
import {IStrategyFactory} from "../../interfaces/plexus/IStrategyFactory.sol";
import {IStrategyConcLiq} from "../../interfaces/plexus/IStrategyConcLiq.sol";
import {IStrategyUniswapV3} from "../../interfaces/plexus/IStrategyUniswapV3.sol";
...
contract StrategyPassiveManagerUniswap is
    ...
    IStrategyUniswapV3
{
    ...
    event ChargedFees(uint256 PlexusFeeAmount);
    event ClaimedFees(
        uint256 feeMain0,
        uint256 feeMain1,
        uint256 feeAlt0,
    ...
    /// @notice Internal function to claim fees from the pool, charge fees for Plexus, then readjust our positions.
    function _harvest(address _callFeeRecipient) private {
        // Claim fees from the pool and collect them.
        _claimEarnings();
        _removeLiquidity();

        // Charge fees for Plexus and send them to the appropriate addresses, charge fees to accrued state fee amounts.
        (uint256 fee0, uint256 fee1) = _chargeFees(
            _callFeeRecipient,
            fees0,
            fees1
        );
    ...
    function claimEarnings()
        external
        returns (uint256 fee0, uint256 fee1, uint256 feeAlt0, uint256 feeAlt1)
    {
    ...
    /**
     * @notice Internal function to charge fees for Plexus and send them to the appropriate addresses.
     * @param _callFeeRecipient The address to send the call fee to.
    ...
    function _chargeFees(
        ...
        if (_amount0 > 0) {
            ...
            uint256 out0;
            if (lpToken0 != native)
                out0 = IPlexusSwapper(unirouter).swap(
                    lpToken0,
                    native,
                    amountToSwap0
                );

            // Add the native earned to the total of native we earned for Plexus fees, handle if token0 is native.
        ...
        if (_amount1 > 0) {
            ...
            uint256 out1;
            if (lpToken1 != native)
                out1 = IPlexusSwapper(unirouter).swap(
                    lpToken1,
                    native,
                    amountToSwap1
                );

            // Add the native earned to the total of native we earned for Plexus fees, handle if token1 is native.
            if (lpToken1 == native) nativeEarned += amountToSwap1;
            else nativeEarned += out1;
        }
        IERC20Metadata(native).safeTransfer(plexusFeeRecipient(), nativeEarned);

        emit ChargedFees(nativeEarned);
    }
        ...
        )
    {
        ...
        (uint128 liquidity, , , uint256 owed0, uint256 owed1) = IUniswapV3Pool(
            pool
        ).positions(keyMain);
        (
    ...
    function retireVault() external onlyOwner {
        if (IPlexusVaultConcLiq(vault).totalSupply() != 10 ** 3)
            revert NotAuthorized();
        panic(0, 0);
        address feeRecipient = plexusFeeRecipient();

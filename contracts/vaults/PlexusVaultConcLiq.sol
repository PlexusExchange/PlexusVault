 ...
 */
contract PlexusVaultConcLiq is
    ...
    ReentrancyGuardUpgradeable
{
        ...
        )
    {
        ...
        (amount0, amount1, fee0, fee1) = _getTokensRequired(
            price,
            _amount0,
            _amount1,
            bal0,
        ...
        )
    {
        ...
        if (_bal1 < bal0InBal1) {
            ...
            if (owedAmount0 > _amount0) {
                ...
                depositAmount1 =
                    _amount1 -
                    (((owedAmount0 - _amount0) * _price) / PRECISION);
            } else {
                depositAmount0 = owedAmount0;
                depositAmount1 = _amount1;
            }

            uint256 fill = _amount1 < (bal0InBal1 - _bal1)
                ? _amount1
                : (bal0InBal1 - _bal1);
            uint256 slidingFee = (bal0InBal1 *
                PRECISION +
                (owedAmount0 * _price)) /
                (bal0InBal1 +
                    _bal1 +
                    fill +
                    ((2 * owedAmount0 * _price) / PRECISION));

            feeAmount1 = (fill * ((_swapFee * slidingFee) / PRECISION)) / 1e18;
        } else {
            uint256 owedAmount1 = bal0InBal1 +
                ((_amount0 * _price) / PRECISION) >
                _bal1
            ...
            if (owedAmount1 > _amount1) {
                depositAmount0 =
                    _amount0 -
                    (((owedAmount1 - _amount1) * PRECISION) / _price);
                depositAmount1 = _amount1;
            } else {
                depositAmount0 = _amount0;
    ...
    function deposit(
        ...
        strategy.deposit();
        uint256 shares = (amount1 - fee1) +
            (((amount0 - fee0) * price) / PRECISION);
        ...
        if (_totalSupply > 0) {
            ...
            shares =
                (shares * _totalSupply) /
                (((((_bal0 + fee0) * price) + PRECISION - 1) / PRECISION) +
                    (_bal1 + fee1));
        } else {
            // First user donates MINIMUM_SHARES for security of the vault.
            shares = shares - MINIMUM_SHARES;

...
import { odosCall } from "../scripts/odosData.js";
...
describe("VaultTest", function () {
  ...
  it("should odos swap-deposit to zapRouter", async function () {
    ...
    const inputTokens = [
      {
        tokenAddress: ZERO, // odos api native
        amount: "10000000000000000000", // ethers.parseUnits("10", 18),
      },
      {
    ...
    const outputTokens = [
      {
        tokenAddress: USDT_ADDRESS, // checksummed output token address
        proportion: 1,
      },
    ];

    const odosCallData = await odosCall(
    ...
    console.log("odosCallData", odosCallData);
    ...
    console.log("before execute", odosCallData.transaction.value);
    const route = [
      {
        target: odosCallData.transaction.to,
        value: odosCallData.transaction.value,
        data: odosCallData.transaction.data,
        tokens: [
          { token: ZERO, index: -1 },
    ...
    await plexusZapRouter
      .connect(mainUser)
      .executeOrder(order, route, { value: odosCallData.transaction.value });

    console.log("zapRouter deposit clear");
  });

  //USDC,MATIC -> 1inch *2 swap -> USDT pool
  it("should 1inch swap-deposit to zapRouter", async function () {
    console.log((await ethers.provider.getBlock("latest"))?.number);
    // instantly mine 1000 blocks
    await mine(1000);
    console.log((await ethers.provider.getBlock("latest"))?.number);

    const oneInchCallData1 = await fetchOneInchData(
    ...
    const oneInchCallData2 = await fetchOneInchData(
      //1inch swap하고 받는곳 zapRouter로 설정.
      await plexusZapRouter.getAddress(),
      NATIVE,
      USDT_ADDRESS,
    ...
    console.log("oneInchCallData1", oneInchCallData1);
    console.log("oneInchCallData2", oneInchCallData2);
    ...
    const route = [
      {
        target: oneInchCallData1.tx.to,
        value: oneInchCallData1.tx.value,
        data: oneInchCallData1.tx.data,
        tokens: [{ token: USDC_ADDRESS, index: -1 }],
      },
      {
        target: oneInchCallData2.tx.to,
        value: oneInchCallData2.tx.value,
        data: oneInchCallData2.tx.data,
        tokens: [{ token: ZERO, index: -1 }],
      },
      {
        target: testPlexusVaultERC20_address,
        value: "0",
